...
static int hf_greybus_i2c_transfer = -1;
...
dissect_greybus(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree _U_, void *data _U_)
{
    ...
    case GB_I2C_TYPE_FUNCTIONALITY:
    	if (response) {
            ...
            proto_tree_add_item(greybus_tree, hf_greybus_i2c_functionality, tvb, offset, 4, ENC_LITTLE_ENDIAN);
            offset += 4;
    	}
    	break;
    case GB_I2C_TYPE_TRANSFER:
    	if (response) {
			...
			proto_tree_add_item(greybus_tree, hf_greybus_i2c_transfer, tvb, offset, remaining, ENC_NA);
			offset += remaining;
    	} else {
			proto_tree_add_item(greybus_tree, hf_greybus_i2c_transfer_addr, tvb, offset, 2, ENC_LITTLE_ENDIAN);
			offset += 2;
			proto_tree_add_item(greybus_tree, hf_greybus_i2c_transfer_flags, tvb, offset, 2, ENC_LITTLE_ENDIAN);
			offset += 2;
